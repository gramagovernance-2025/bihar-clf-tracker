...
// "98th percentile · out of 64 CLFs" -> "98th percentile · 2nd out of 64 CLFs" when a
// rank is available; falls back to the plain phrasing when it isn't (composite/derived
// scores with no real peer population to rank against, e.g. Scoring's category totals).
function pctlSubtext(rank, n){
  if(rank==null || n==null) return n!=null ? `percentile · out of ${n} CLFs` : 'percentile';
  return `percentile · ${rank}${ord(rank)} out of ${n} CLFs`;
}
function standingCard(kind,big,subText,rankLine,labelText){
  const known = big!==null && big!==undefined;
  const pct = known ? big : 0;
  return `<div class="standing-card ${kind}"><div class="label">${labelText}</div><div class="big serif">${known?big:'—'}<sup>${known?ord(big):''}</sup></div>
    <div class="sub">${subText}</div><div class="track"><div class="fill" style="width:${pct}%"></div><div class="marker" style="left:${pct}%"></div></div>
...
function pctlRow(label, m, tip){
  ...
  const dRank = m.district_rank!=null && m.n_district!=null ? ` (${m.district_rank}${ord(m.district_rank)} of ${m.n_district})` : '';
  const sRank = m.state_rank!=null && m.n_state!=null ? ` (${m.state_rank}${ord(m.state_rank)} of ${m.n_state})` : '';
  return `<div class="metric-pctl-row" style="display:grid;grid-template-columns:190px 1fr 1fr;gap:16px;align-items:center;margin-bottom:12px;">
    ...
    <div><div style="font-size:11px;color:var(--ink-soft);display:flex;justify-content:space-between;margin-bottom:4px;"><span style="color:var(--primary);font-weight:700;">DISTRICT</span><span>${withOrd(m.district_pctl)}${dRank}</span></div>
      <div class="bar-track"><div class="fill" style="width:${m.district_pctl||0}%;background:var(--primary)"></div></div></div>
    <div><div style="font-size:11px;color:var(--ink-soft);display:flex;justify-content:space-between;margin-bottom:4px;"><span style="color:var(--gold);font-weight:700;">STATE</span><span>${withOrd(m.state_pctl)}${sRank}</span></div>
...
function renderOverview(sub){
  document.getElementById('panel-overview').innerHTML =
    ...
    `<div class="foot-note">Data source: ${FOOTNOTES.overview}</div>`;
  if(sub==='members'){
    const lvColors = ['var(--primary)','var(--gold)','#5B8AA6','var(--low)','var(--grey)'];
  ...
  <section><div class="section-head"><h2 class="serif">Grade &amp; Standing</h2><span class="hint">Q4 only</span></div>
    <div class="panel">
      ...
      <div class="standing-grid">
        ${standingCard('district', a.district_pctl, pctlSubtext(a.district_rank, a.n_district), 'Score '+a.total_score+' / 100', 'Standing vs. District')}
        ${standingCard('state', a.state_pctl, pctlSubtext(a.state_rank, a.n_state), 'Score '+a.total_score+' / 100', 'Standing vs. State')}
      </div>
    </div></section>
...
function renderAuditIrreg(){
  ...
  const textBlock = (a.issue_flagged && a.irreg_text) ?
    `<p class="desc" style="max-width:none;margin-top:14px;padding:12px 14px;background:var(--panel-alt);border-radius:8px;border:1px solid var(--line);">"${a.irreg_text}"</p>` : '';
...
function renderAudit(){
  document.getElementById('panel-audit').innerHTML =
    ...
    `<div class="foot-note">Data source: ${FOOTNOTES.audit}</div>`;
...
JS_FINANCIAL = r"""
let finQtrIdx = 0;
const F01_COLORS = ['var(--gold)','var(--primary)','#5B8AA6'];
...
function renderFinSummary(){
  ...
  const capDonut = (f01.found && isCurrentSnapshot) ? donutBlock('ring-capital', f01.capital_composition, F01_COLORS, {size:140, fmt:v=>v+'%'}) : null;
  const f01Block = !f01.found ? `
    ...
    <section><div class="section-head"><h2 class="serif">Balance Sheet</h2><span class="hint">as of ${f01.period}</span></div>
      <div class="panel"><div class="tiles n4">
        ${tile('Books Balance Check', f01.balance_gap_pct!=null?fmtF(f01.balance_gap_pct,2)+'% gap':'—', 'Assets vs. Liabilities+Equity', f01.balance_gap_pct!=null&&f01.balance_gap_pct!==0?'neg':'')}
      ...
      <div class="section-head" style="margin:18px 0 10px;"><h3 style="font-size:14px;margin:0;" class="tip" data-tip="${TIPS['Where the Fund Came From']}">Where the Fund Came From</h3></div>
      ${capDonut.html}
      </div></section>` :
    `<section><div class="section-head"><h2 class="serif">Balance Sheet</h2></div>
  ...
  const rc = q.receipts_full ? Object.fromEntries(Object.entries(q.receipts_full).filter(([k])=>k!=='Total Receipts')) : null;
  const pc = q.payments_full ? Object.fromEntries(Object.entries(q.payments_full).filter(([k])=>k!=='Total Payments')) : null;
  ...
  return { html, capDonut, rDonut, pDonut };
}
function renderFinStatements(){
  ...
  <section><div class="section-head"><h2 class="serif tip" data-tip="${TIPS['Balance Sheet']}">Balance Sheet</h2><span class="hint">${(f01.found && isCurrentSnapshot)?('as of '+f01.period):q.label}</span></div>
    <div class="panel">
      ...
      <div style="margin-top:18px;"><h3 style="font-size:13.5px;margin:0 0 8px;">Capital Composition</h3>${tableHtml([{label:'Source'},{label:'Share',num:true}], Object.entries(f01.capital_composition).map(([k,v])=>[k, v!=null?v+'%':'—']))}</div>` :
...
function renderFinancial(sub){
  ...
  document.getElementById('panel-financial').innerHTML =
    dropdown + body + `<div class="foot-note">Data source: ${FOOTNOTES.financial}</div>`;
  ...
  if(sub==='summary' && summaryDraws){
    if(summaryDraws.capDonut) summaryDraws.capDonut.draw();
...
function renderVrfFundStanding(){
  ...
  return `<section><div class="section-head"><h2 class="serif">Fund Standing</h2><span class="hint">composite of 5 fund-health metrics, member-weighted, percentile-ranked</span></div>
    <div class="panel"><div class="standing-grid">
      ${standingCard('district', v.composite_dist_pctl, pctlSubtext(v.composite_dist_rank, v.n_district), 'Composite score '+v.composite, 'Within '+v.district_name+' District')}
      ${standingCard('state', v.composite_state_pctl, pctlSubtext(v.composite_state_rank, v.n_state), 'Composite score '+v.composite, 'Within Bihar (Statewide)')}
    </div>
...
function renderVRF(sub){
  if(!DATA.vrf.found){
    document.getElementById('panel-vrf').innerHTML = `<section><div class="panel"><p class="disclaimer">${ERR_MSG_JS.vrf}</p></div></section>`;
    return;
  }
  const fn = {kpi: renderVrfKpi, vobreak: renderVrfVO, bkrank: renderVrfBK, forecasts: renderVrfForecasts}[sub];
  document.getElementById('panel-vrf').innerHTML = fn() + `<div class="foot-note">Data source: ${FOOTNOTES.vrf}</div>`;
  if(sub==='kpi'){
    const v = DATA.vrf;
...
function renderVprpEnt(){
  ...
  const pendingNote = vprpYear===2025 ? ` — shown as <b>pending</b>, not 0%, since this year is too recent to have been processed` : '';
  const schemeRows = (y.by_scheme||[]).map(s=>{
    const tds = `<td>${s.scheme}</td><td class="num">${fmtNum(s.demanded)}</td><td class="num">${vprpYear===2025?'<i>pending</i>':fmtNum(s.accessed)}</td>`;
    let extra = '';
    if(s.raw_scheme==='state-specific' && y.state_scheme_breakdown){
      const subRows = Object.entries(y.state_scheme_breakdown).map(([k,v])=>`<tr><td style="padding-left:28px;color:var(--ink-soft);">${k}</td><td class="num" colspan="2">${fmtNum(v)}</td></tr>`).join('');
      extra = `<tr><td colspan="3" style="padding:2px 0 8px 10px;"><table style="width:100%;"><tbody>${subRows}</tbody></table></td></tr>`;
    }
  ...
  return `<section><div class="section-head"><h2 class="serif">Entitlements</h2></div>
    <div class="panel">
      <div class="tiles n3" style="margin-bottom:16px;">
        ${tile('Total Entitlement Demands', fmtNum(y.n_demands))}
        ${tile('Schemes Accessed', vprpYear===2025?'—':fmtNum(y.accessed))}
        ${tile('Schemes Still Pending', vprpYear===2025?fmtNum(y.n_demands):fmtNum(y.pending))}
      </div>
      ${y.by_scheme ? `<p class="hint tip" style="margin-bottom:8px;" data-tip="${TIPS['Demand & Access by Scheme']}"><b>Demand &amp; Access by Scheme</b>${pendingNote}</p>
        <div class="table-wrap"><table><thead><tr><th>Scheme</th><th class="num">Demanded</th><th class="num">Accessed</th></tr></thead><tbody>${schemeRows}</tbody></table></div>` : ''}
    </div></section>`;
}
...
function renderVPRP(sub){
  ...
  document.getElementById('panel-vprp').innerHTML = yearDropdown() + body + `<div class="foot-note">Data source: ${FOOTNOTES.vprp}</div>`;
...
function renderScoringOverall(){
  ...
  return `<section><div class="section-head"><h2 class="serif tip" data-tip="${TIPS['Overall Score']}">Overall Score</h2><span class="hint">equal-weighted average across every individual item, pooled across all 5 categories</span></div>
    <div class="panel">
      <div class="tiles n1" style="margin-bottom:16px;">${tile('Overall Score', s.overall_score+' / 100', null, 'big-value')}</div>
      <div class="standing-grid">
        ${standingCard('district', s.overall_district_score, pctlSubtext(s.overall_district_rank, s.overall_n_district), 'Overall Score', 'Standing vs. District')}
        ${standingCard('state', s.overall_score, pctlSubtext(s.overall_state_rank, s.overall_n_state), 'Overall Score', 'Standing vs. State')}
      </div></div></section>
...
function renderScoring(sub){
  ...
  document.getElementById('panel-scoring').innerHTML =
    dropdown + (sub==='overall' ? renderScoringOverall() : renderScoringByCategory()) +
    `<div class="foot-note">Data source: ${FOOTNOTES.scoring}</div>`;
...
async function initShell(){
  ...
  MANIFEST = manifest; TIPS = shared.tips || {}; ERR_MSG_JS = shared.err_msg || {}; FOOTNOTES = shared.footnotes || {};

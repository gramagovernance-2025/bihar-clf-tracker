...
function pctlSubtext(rank, n, pctl){
  const pctlPart = pctl!=null ? `${pctl}${ord(pctl)} percentile` : 'percentile';
  if(rank==null || n==null) return pctlPart;
  return `${rank}${ord(rank)} out of ${n} CLFs · ${pctlPart}`;
}
  ...
  <section><div class="section-head"><h2 class="serif">Grade &amp; Standing</h2><span class="hint">Q4 only</span></div>
    <div class="panel">
      ...
      <div class="standing-grid">
        ${standingCard('district', a.district_pctl, pctlSubtext(a.district_rank, a.n_district, a.district_pctl), 'Score '+a.total_score+' / 100', 'Standing vs. District')}
        ${standingCard('state', a.state_pctl, pctlSubtext(a.state_rank, a.n_state, a.state_pctl), 'Score '+a.total_score+' / 100', 'Standing vs. State')}
      </div>
    </div></section>
...
function renderVrfFundStanding(){
  ...
  return `<section><div class="section-head"><h2 class="serif">Fund Standing</h2><span class="hint">composite of 5 fund-health metrics, member-weighted, percentile-ranked</span></div>
    <div class="panel"><div class="standing-grid">
      ${standingCard('district', v.composite_dist_pctl, pctlSubtext(v.composite_dist_rank, v.n_district, v.composite_dist_pctl), 'Composite score '+v.composite, 'Within '+v.district_name+' District')}
      ${standingCard('state', v.composite_state_pctl, pctlSubtext(v.composite_state_rank, v.n_state, v.composite_state_pctl), 'Composite score '+v.composite, 'Within Bihar (Statewide)')}
    </div>
...
function renderScoringOverall(){
  ...
  return `<section><div class="section-head"><h2 class="serif tip" data-tip="${TIPS['Overall Score']}">Overall Score</h2><span class="hint">equal weight across all 5 categories by default - customize below</span></div>
    <div class="panel">
      ...
      <div class="standing-grid">
        ${standingCard('district', s.overall_district_score, pctlSubtext(s.overall_district_rank, s.overall_n_district, s.overall_district_score), 'Overall Score', 'Standing vs. District')}
        ${standingCard('state', s.overall_score, pctlSubtext(s.overall_state_rank, s.overall_n_state, s.overall_score), 'Overall Score', 'Standing vs. State')}
      </div></div></section>

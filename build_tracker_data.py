...
def build_clf_data(mis_id, clfcode, district, block, audit_name):
    ...
    if F01_FOUND:
        ...
        gov = r01['Received From SRLM'] + r01['Advance from SRLM']; member_cap = r01['Capital Contribution']
        inst = r01['Loan from Banks & Other Institutions'] + r01['Loan From Nodal CLF']
        cap_total = gov + member_cap + inst
        f01_data = {
            "found": True,
            "period": r01['period'],
            ...
            "capital_composition": {"Government": round(gov / cap_total * 100, 1), "Member Capital": round(member_cap / cap_total * 100, 1),
                                     "Institutional": round(inst / cap_total * 100, 1)},
            "assets": {
        ...
        f01_data = {"found": False, "deployment_ratio": None, "surplus_pct": None, "balance_gap_pct": None}
    ...
    for yr in [2023, 2024, 2025]:
        ...
        if len(e_y):
            ...
            yr_data["accessed"] = accessed
            yr_data["pending"] = len(e_y) - accessed
            by_scheme = e_y.groupby('scheme_type').agg(demanded=('nrega_accessed', 'size'), accessed=('nrega_accessed', 'sum')).sort_values('demanded', ascending=False)
            by_scheme = by_scheme[by_scheme['demanded'] > 0]
            yr_data["by_scheme"] = [{"scheme": SCHEME_LABELS.get(s, s), "raw_scheme": s, "demanded": int(r['demanded']), "accessed": int(r['accessed'])} for s, r in by_scheme.head(6).iterrows()]
    ...
    for _qi, (_fy, _q, _label) in enumerate(QUARTERS):
        ...
        all_metrics = fund_util_metrics + financial_health_metrics + vrf_health_metrics + governance_metrics + social_metrics
        state_item_vals = [m[1]['state_pctl'] for m in all_metrics if m[1] and m[1].get('state_pctl') is not None]
        dist_item_vals = [m[1]['district_pctl'] for m in all_metrics if m[1] and m[1].get('district_pctl') is not None]
        overall_score = round(sum(state_item_vals) / len(state_item_vals)) if state_item_vals else None
        overall_district_score = round(sum(dist_item_vals) / len(dist_item_vals)) if dist_item_vals else None
...
with open(f"{DATA_DIR}/shared.json", "w") as f:
    json.dump({"tips": TIPS, "err_msg": ERR_MSG, "footnotes": FOOTNOTES}, f)
print(f"[{elapsed()}] Wrote shared.json (TIPS/ERR_MSG/FOOTNOTES)")
